...
import 'package:petrolpump/API_Services/cusOutstanding_services.dart';
...
class CustomerOutstanding extends StatefulWidget {
  final String title;
  const CustomerOutstanding({super.key, this.title = "Customer Outstanding"});
...
class _CustomerOutstandingState extends State<CustomerOutstanding> {
  ...
  Widget build(BuildContext context) {
    ...
    final customerOutstandingProvider =
        Provider.of<CustomerOutstandingProvider>(context);

    if (_shouldResetState) {
      customerOutstandingProvider.clearSelection();
      customerOutstandingProvider.selectedMR = '';
      customerOutstandingProvider.selectedMRID = 0;
      customerOutstandingProvider.selectedProductCompany = '';
      customerOutstandingProvider.selectedProductCompanyID = 0;
      customerOutstandingProvider.selectedArea = '';
      customerOutstandingProvider.selectedAreaID = 0;
      customerOutstandingProvider.clearSelectionDate();
        ...
        actions: [
          Row(
            children: [
              IconButton(
                  onPressed: () {
                    if (customerOutstandingProvider.ledgerName.isNotEmpty) {
                      _displayPdf();
                    } else {
                        ...
                        Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Consumer<CustomerOutstandingProvider>(
                                builder: (context, value, child) {
                                  return InkWell(
                                    onTap: () async {
  ...
  void _displayPdf() async {
          ...
          return [
            ...
            pw.Column(
              children: [
                ...
                pw.Text(
                  customerOutstandingProvider.ledgerName,
                  style: pw.TextStyle(
                    fontSize: 12.sp,
                    fontWeight: pw.FontWeight.bold,
                  ),
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PreviewScreenCusOutstanding(doc: doc),
      ),
    );
  }
}

class PreviewScreenCusOutstanding extends StatelessWidget {
  final pw.Document doc;

  const PreviewScreenCusOutstanding({
    Key? key,
    required this.doc,
  }) : super(key: key);
